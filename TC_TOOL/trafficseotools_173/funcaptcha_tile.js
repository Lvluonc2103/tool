(async () => {
  ...
  function p() {
    ...
    var t;
    return e
      ? e.src?.split(";base64,")[1]
      : (e = l([".challenge-container button"])) &&
        (t = window
          .getComputedStyle(e, null)
          ?.backgroundImage?.trim()
          ?.match(/(?!^)".*?"/g)) &&
        0 !== t.length
      ? (t = t[0].replaceAll('"', "")).startsWith("blob:")
        ? t
        : t.split(";base64,")[1]
      : null;
  }
  ...
  async function e(e) {
          ...
          if (!i) {
            ...
            if (t && t.enabled && t.funcaptcha_auto_solve) {
              ...
              if (t) {
                var a = l(
                  [
                    "#game_children_challenge ul > li > a",
    ...
    if (null !== a && null !== n && 0 !== i.length && null !== i[0])
      if ("Pick any square" === a)
        (o = (o = parseInt(e.funcaptcha_solve_delay_time)) || 1e3),
          await Time.sleep(o),
          (d = null),
          Input.click(n[0]);
      else {
        var o = "funcaptcha",
          { start: a, data: c } = await NopeCHA.post({
            settings: e,
            type: o,
            task: a,
            image_data: i,
        ...
        if (c) {
          await NopeCHA.delay({ settings: e, start: a, type: o });
  ...
  if (
    ...
    window.location.pathname.startsWith("/fc/assets/tile-game-ui/") ||
      window.location.pathname.startsWith("/fc/assets/ec-game-core/"))
  )
    for (;;) {
      await Time.sleep(1e3);
      var t,
        a = await BG.exec("Settings.get");
      a &&
        a.enabled &&
        ((t = await Location.hostname()),
        a.disabled_hosts.includes(t) ||
          (a.funcaptcha_auto_open && r()
            ? await s()
            : a.funcaptcha_auto_solve &&
              null !== u() &&
              null !== p() &&
              (await e(a))));
    }
